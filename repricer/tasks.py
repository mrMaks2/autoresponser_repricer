...
seller_arts = load_dict_from_file('repricer/seller_arts.txt')
prod_arts_cab1 = load_dict_from_file('repricer/prod_arts_cab1.txt')
prod_arts_cab2 = load_dict_from_file('repricer/prod_arts_cab2.txt')
prices_dict_cab1 = load_dict_from_file('repricer/prices_dict_cab1.txt')
prices_dict_cab2 = load_dict_from_file('repricer/prices_dict_cab2.txt')
...
def process_product(wb_arts, ozon_arts, prices_with_discount_wb, prices_with_discount_ozon, headers_for_ozon):
    ...
    discount_co_invest = round(1 - price_with_co_invest / price_without_co_invest, 5)
    
    if price_with_co_invest == 0:
        logger.warning(f"Цена с соинвестом равна 0 для продукта {offer_id}")
        return None
    
    discount_ozon_with_wallet = round(1 - price_with_discount_ozon / price_with_co_invest, 5)
    ...
    if price_with_discount_ozon == price_ozon_s_be_with_wallet:
        logger.info(f"Цена товара Ozon {ozon_arts[0]} равна желаемой")
        return None
    ...
    for wb_art, ozon_art in seller_arts.items():
        ...
        for wb_articul, wb_price in prices_dict.items():
            ...
            if old_price and not (wb_price - int(span) <= old_price <= wb_price + int(span)):
                ...
                if new_price_for_wb:
                    logger.info(f"Старая цена {wb_articul} на WB: {wb_old_price}")
                    logger.info(f"Новая цена {wb_articul} для WB: {new_price_for_wb['price']}")
                ...
                logger.info(f"Цена товара {wb_articul} на WB находится в диапозоне {wb_price}+-{int(span)}")
                continue
            else:
                logger.warning(f"Нет данных по цене с парсенных результатов: {wb_articul}")
                continue

        if params_for_wb_post_all['data']:
            res_for_wb = requests.post(url_wb_post, headers=headers_for_wb, json=params_for_wb_post_all)
            res_for_wb_data = res_for_wb.json()
            logger.info(f"Статус ответа: {res_for_wb.status_code}")
        
            if res_for_wb.status_code in (200, 208):
                logger.info(f"Изменены ли цены товаров по загрузке {res_for_wb_data['data']['id']} на WB: {not res_for_wb_data['data']['alreadyExists']}")
                if res_for_wb_data['data']['alreadyExists'] == True:
                    logger.info(f"Ошибки: {res_for_wb_data['errorText']}")

            try:
                id_upload_status = res_for_wb_data['data']['id']
            ...
            res_for_wb = None
            logger.info('Пустой список для изменения цен. POST запрос на WB не был отправлен')
        ...
        try:
            if res_for_wb:
                ...
                prices_with_discount_wb = prices_with_discount_wb_price
                prices_with_discount_ozon = parse_from_ozon(ozon_art)
        ...
        if params_for_ozon_post_all['prices']:
            ...
            if res_for_ozon.status_code == 200:
                for i in range(len(res_for_ozon.json()['result'])):
                    res_for_ozon_i = res_for_ozon.json()['result'][i]
                    logger.info(f"Изменена ли цена товара {res_for_ozon_i['offer_id']} на Ozon: {res_for_ozon_i['updated']}")
                    if res_for_ozon_i['updated'] == False:
                        logger.info(f"Ошибки: {res_for_ozon_i['errors']}")
            else:
                logger.info(f"Код ошибки: {res_for_ozon.json()['code']}, сообщение ошибки: {res_for_ozon.json()['message']}")
        else:
            logger.info(f'Пустой список для изменения цен. POST запрос на Ozon не был отправлен')
